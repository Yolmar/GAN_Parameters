...
data_name = "Data_Augmented_3k"
experiment_name = 'Test'
epoch_snapshot = [1, 5, 10, 20, 30, 40, 50, 60, 70]
batch_sizes = [8, 16, 32, 64, 128, 256]
lrs = [2*1e-4, 1e-4, 2*1e-5, 1e-5]

for epoch in epoch_snapshot:
    modelroot = f"Models/{experiment_name}/e{epoch}.pth"
    ...
    if not isModelExist:
        ...
        save_path = f'Plots&Images/{experiment_name}/Generated_Images'
        ...
        plt.ylabel("Loss")
        plt.legend()
        save_path = f'Plots&Images/{experiment_name}/G&D_Loss'
        isExist = os.path.exists(save_path)
        if not isExist:
            os.makedirs(save_path)
        plt.savefig(f'{save_path}/e{epoch}.png')
